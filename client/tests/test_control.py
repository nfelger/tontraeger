...
async def test_handle_present_plays_uri(cache) -> None:
    cache.update([{"tag_uid": "tag1", "media_uri": "x-sonosapi-radio:s25111", "name": ""}])
    sonos = DummySonosAPI()
    controller = PlaybackController(sonos, cache)  # type: ignore[arg-type]

    await controller.handle_present("tag1")
...
async def test_handle_present_unknown_reports(cache) -> None:
    sonos = DummySonosAPI()
    sync = DummySync()
    controller = PlaybackController(sonos, cache, sync=sync)  # type: ignore[arg-type]
...
async def test_handle_present_unknown_without_sync(cache) -> None:
    sonos = DummySonosAPI()
    controller = PlaybackController(sonos, cache, sync=None)  # type: ignore[arg-type]
...
async def test_handle_removed_pauses(cache) -> None:
    ...
    await controller.handle_removed("tag1")
...
async def _run_nfc_reader_with_fake_daemon(
    ...
    Returns the controller, sonos fake, and last process fake for assertions.
    """
    if sonos is None:
        sonos = DummySonosAPI()
    controller = PlaybackController(sonos, cache, sync=sync)  # type: ignore[arg-type]
...
async def test_nfc_reader_restarts_on_eof(cache) -> None:
    ...
    lines: list[bytes] = []  # immediate EOF
    restart_count = 0

    sonos = DummySonosAPI()
    controller = PlaybackController(sonos, cache)  # type: ignore[arg-type]

    async def counting_subprocess_exec(*args, **kwargs):  # type: ignore[no-untyped-def]
        nonlocal restart_count
        restart_count += 1
        if restart_count >= 3:
            raise asyncio.CancelledError()
...
async def test_nfc_reader_terminates_child_on_exit(cache) -> None:
    """When cancelled while the daemon is still running, terminate() is called."""
...
async def test_nfc_reader_backoff_resets_after_output(cache) -> None:
    ...
    async def fake_subprocess_exec(*args, **kwargs):  # type: ignore[no-untyped-def]
        nonlocal call_count
        call_count += 1
        if call_count == 1:
